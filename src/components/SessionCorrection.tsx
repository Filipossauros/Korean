...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  'gramática': 'bg-ink/10 text-ink/70',
  'tempo_verbal': 'bg-jade/10 text-jade',
  'registo': 'bg-gold/10 text-gold',
  'ordem_palavras': 'bg-ink/10 text-ink/70',
}
  ...
  return (
    <div className="min-h-screen bg-paper pb-24 md:pb-0">
      <div className="max-w-2xl mx-auto px-4 py-6">
        {/* Score summary */}
        <div className="bg-white rounded-2xl p-5 border border-line mb-6 text-center">
          <p className="text-5xl font-bold font-ui text-ink mb-1">{total}<span className="text-2xl text-ink/30">/20</span></p>
          ...
          <p className="text-sm text-ink/50 font-ui">{pct}% · {sessao.tema}</p>
        </div>

        {/* Part 1 correction */}
        <div className="mb-6">
          <div className="flex justify-between items-center mb-3">
            <h2 className="font-ui font-semibold text-ink">Leitura</h2>
            <span className="font-ui text-sm font-bold text-ink">{sessao.parte1.pontuacao}/10</span>
          </div>
          <div className="bg-white rounded-2xl p-4 border border-line">
            <p className="text-xs text-ink/40 font-ui mb-1">A tua tradução:</p>
            <p className="font-ui text-sm text-ink mb-3">{sessao.parte1.traducao_utilizador || '—'}</p>
            {sessao.parte1.traducao_referencia && (
              <>
                <p className="text-xs text-ink/40 font-ui mb-1">Tradução de referência:</p>
              ...
              <div className="mt-3 space-y-2">
                {sessao.parte1.erros.map((e, i) => (
                  <div key={i} className="flex gap-2 items-start text-xs">
                    <span className={`px-2 py-0.5 rounded-lg font-ui shrink-0 ${CATEGORY_COLORS[e.categoria] ?? 'bg-line text-ink/50'}`}>
                    ...
                    <div>
                      ...
                      {e.nota && <p className="text-ink/50 mt-0.5">{e.nota}</p>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-3">
            <h2 className="font-ui font-semibold text-ink">Produção</h2>
            <span className="font-ui text-sm font-bold text-ink">{sessao.parte2.pontuacao}/10</span>
          </div>
          <div className="space-y-3">
            {sessao.parte2.frases.map((f, i) => (
              <div key={i} className={`bg-white rounded-xl p-4 border ${f.correcto ? 'border-jade/30' : 'border-vermillion/30'}`}>
                <div className="flex gap-2 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-ink/40 font-ui">{f.pt_original}</p>
                    <p className="font-serif text-base text-ink mt-0.5">{f.kr_utilizador || '—'}</p>
                    {!f.correcto && f.kr_referencia && (
                      <p className="font-serif text-sm text-jade mt-1">{f.kr_referencia}</p>
                    )}
                    {f.nota && <p className="text-xs text-ink/50 font-ui mt-1">{f.nota}</p>}
                    {!f.correcto && f.categoria_erro && (
                      <span className={`text-xs px-2 py-0.5 rounded-lg font-ui mt-1 inline-block ${CATEGORY_COLORS[f.categoria_erro] ?? 'bg-line text-ink/50'}`}>
        ...
        {loading && (
          <div className="text-center py-4">
            <p className="text-ink/40 font-ui text-sm animate-pulse">A corrigir…</p>
          </div>
        )}
